...
Se por alguma razão precisar da um cleanup nos volumes e na aplicação em si, sinta-se a vontade de usar o comando:
...
Embora o Redis seja comum para dados temporários devido à sua performance, ele não oferece garantias transacionais ACID nem executa regras de negócio ao expirar uma chave (ex.: publicar eventos ou atualizar estados relacionados). Como as reservas representam um estado crítico do domínio – envolvendo concorrência e edge cases como race conditions –, optou-se por persisti-las no PostgreSQL. Isso permite manter um histórico confiável de tentativas de reserva, prevenindo inconsistências em cenários de indisponibilidade do Redis e facilitando processamento em batch com locks para evitar deadlocks.
