...
O RabbitMQ foi selecionado por oferecer funcionalidades nativas importantes, como Dead Letter Queue, que auxiliam no tratamento de mensagens expiradas e permitem lidar com expirações automáticas de reservas e retentativas de processamento de forma resiliente.
...
ADENDO: Se estiver usando linux lembre-se de utilizar o comando "sudo" para conseguir executar de forma apropriada

### AVISOS

Se por alguma razão precisar dar um cleanup nos volumes e na aplicação em si, sinta-se a vontade de usar o comando:

```bash
npm  run  docker:cleanup
```

Fica anotado também que, ao fazer as requests via documentação do swagger, é importante entender que pode ter um pequeno delay de 1 as 2 segundos quando se altera algum parametro ou body da requisição que você está prestes a fazer, isso se dá devido a natureza do ScalarAPI e mesmo com todas as atualizações aplicadas ainda se encontra esse pequeno inconveniente. Então caso alguma requisição parece retornar algo errado, tente enviar novamente, pode ser que tenha escrito rápido demais.
...
Uma das principais decisões técnicas foi gerenciar o ciclo de vida das reservas no banco relacional (PostgreSQL), em vez de utilizar o Redis com TTL para expiração automática.

Embora o Redis seja comum para dados temporários devido à sua performance, ele não oferece garantias transacionais ACID e pode muito bem perder todas as notificações de expiração de reserva se a aplicação ficar offline durante um breve momento. Como as reservas representam um estado crítico do domínio – envolvendo concorrência e edge cases como race conditions –, optou-se por persisti-las no PostgreSQL. Isso permite manter um histórico confiável de tentativas de reserva, prevenindo inconsistências em cenários de indisponibilidade do Redis e facilitando processamento em batch com locks para evitar deadlocks.
...
- Implementar mais testes relacionados ao próprio fluxo de filas do RabbitMQ, preparar um ambiente de testes com reservas que expiram em um tempo menor, e validar se todo o fluxo de mensagens é chamado
...
PUT /customer/pay/{reservationId} 7. Consultar histórico de compras
Acesse a rota para verificar todas as compras já realizadas

GET /customer/list-history 8. Consultar assentos em tempo real
Esta rota mostra os assentos e informações em “tempo real” de uma dada sessão (utilizando do redis)

GET /customer/session/{sessionId} 4. Estratégias implementadas
Como foram tratadas as race conditions?
O principal risco identificado estava na tentativa de reservar o mesmo assento simultaneamente por múltiplos usuários e na atualização concorrente das informações de sessão armazenadas no Redis (por exemplo, atualizar o status de um assento dentro do objeto JSON que representa a sessão).
...
Banco relacional como fonte única de verdade, garantindo consistência forte por meio de transações ACID e locks pessimistas.

Redis como cache sincronizado, com uso de locks distribuídos para evitar que múltiplas instâncias atualizem simultaneamente a mesma representação de sessão.

Mensageria com RabbitMQ, responsável por propagar eventos de alteração de estado, permitindo que todas as instâncias reajam de forma assíncrona e mantenham seus dados derivados atualizados.
...
0 of 69
Markdown 10179 bytes 1457 words 373 lines Ln 336, Col 0HTML 8059 characters 1388 words 102 paragraphs
